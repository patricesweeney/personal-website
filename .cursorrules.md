...
@app.function(
    ...
    image=modal.Image.debian_slim().pip_install("supabase", "pandas"),
)
def process_job(job_id: str):
    # 1. Fetch job from Supabase
    # 2. Update status to "running"
    # 3. Download file, process
    # 4. Write result, update status to "done"
    # 5. Cleanup uploaded file
    pass

# Trigger endpoint (thin)
@app.function()
@modal.web_endpoint(method="POST")
def trigger(payload: dict):
    job_id = payload.get("jobId")
    process_job.spawn(job_id)  # Fire and forget
...
| Rule | Reason |
|------|--------|
| Use `.spawn()` not `.remote()` | Async fire-and-forget; don't block the webhook |
| Jobs must be idempotent | Modal auto-retries on infra failures |
| Always update job status on error | Use try/finally pattern |
| Pin dependency versions | Reproducible builds |
| Store secrets via `modal secret create` | Never in code |
...
modal run modal_app/app.py::fn  # Test single function
```
